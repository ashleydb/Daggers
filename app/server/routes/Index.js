...
function createIndex(_title = null, _description = null, _image = null, _url = null) {
    ...
    var image = (_image) ? _image : 'https://daggers.storage.googleapis.com/basics/News-Generic2_169.jpg';
...
router.route('/news')
    ...
    .get(function(req, res) {
        res.send(createIndex("News", "The latest news from D&amp;R Football Club.", null, "https://www.daggers.co.uk/news"));
    });

router.route('/fixtures')
    // Fixtures homepage
    .get(function(req, res) {
        res.send(createIndex("Fixtures", "Fixtures and results for D&amp;R Football Club.", null, "https://www.daggers.co.uk/fixtures"));
    });

router.route('/table')
    // Fixtures homepage
    .get(function(req, res) {
        res.send(createIndex("Table", "League Table featuring D&amp;R Football Club.", null, "https://www.daggers.co.uk/table"));
    });

router.route('/tickets')
    // Fixtures homepage
    .get(function(req, res) {
        res.send(createIndex("Tickets", "Details on ticket sales for D&amp;R Football Club fixtures.", null, "https://www.daggers.co.uk/tickets"));
    });

router.route('/team')
    // Fixtures homepage
    .get(function(req, res) {
        res.send(createIndex("Team", "Player profiles from D&amp;R Football Club.", null, "https://www.daggers.co.uk/team"));
    });

router.route('/fans')
    // Fixtures homepage
    .get(function(req, res) {
        res.send(createIndex("Fans", "Information for D&amp;R Football Club Fans, including away travel details and match highlights.", null, "https://www.daggers.co.uk/fans"));
    });

router.route('/community')
    // Fixtures homepage
    .get(function(req, res) {
        res.send(createIndex("Community", "The D&amp;R Football Club Community Trust.", null, "https://www.daggers.co.uk/community"));
    });

router.route('/club')
    // Fixtures homepage
    .get(function(req, res) {
        res.send(createIndex("Club", "Club information, including the history of D&amp;R Football Club.", null, "https://www.daggers.co.uk/club"));
    });

router.route('/commercial')
    // Fixtures homepage
    .get(function(req, res) {
        res.send(createIndex("Commercial", "Commercial opportunities, including hospitality and sponsorship with D&amp;R Football Club.", null, "https://www.daggers.co.uk/commercial"));
    });
...
router.route('/news/:news_id')
    ...
    .get(function(req, res) {
        ...
        News.findByIdBrute(options, function(err, news) {
            ...
            res.send(createIndex(news.headline, news.summary, news.image, `https://www.daggers.co.uk/news/${news.id}`));
        });
    });
    ...
    .get(function(req, res) {
        ...
        Pages.findById(req.params.page_id, function(err, page) {
            if (err) {
                ...
                res.send(createIndex(page.name, page.name, null, `https://www.daggers.co.uk/page/${page.id}`));
            }
        });
    });
